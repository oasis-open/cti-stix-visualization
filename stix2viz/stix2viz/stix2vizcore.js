...
export default (d3) => {
  ...
  Viz.prototype.initGraph = function () {
    ...
    var link = this.svg.selectAll('path.link').data(this.currentGraph.edges).enter().append('path')
      ...
      // TODO: I definitely can't see how this is not causing issues. 
  ...
  Viz.prototype.handleSelected = function (d, el) {
    ...
    const jsonString = JSON.stringify(d, selectedReplacer, 2); // get only the STIX values
    const purified = JSON.parse(jsonString); // make a new JSON object from the STIX values
